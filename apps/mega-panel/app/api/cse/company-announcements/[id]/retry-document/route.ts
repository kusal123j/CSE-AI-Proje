...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const url = `${cseBackendBaseUrl()}/api/cse/company-announcements/${params.id}/retry-document`;
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json({ success: false, message: payload && typeof payload === 'object' && 'message' in payload ? String((payload as { message?: unknown }).message) : 'CSE announcement document retry failed', status: response.status, details: payload }, { status: response.status });
    }
